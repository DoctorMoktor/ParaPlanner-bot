...
# === Команда /start (чтобы пользователь мог принимать ЛС) ===
...
async def create_plan(message: types.Message):
    ...
    # === Создаём сообщение только в топике MAIN_THREAD_ID ===
    try:
...
async def join_plan(callback: types.CallbackQuery):
    ...
    if msg_id in plans:
        ...
        # Попытка отправить ЛИЧНОЕ сообщение и корректное ответное уведомление
        try:
            text = (
            ...
            # Если не удалось — скорее всего пользователь не писал боту в ЛС
...
# === Запуск ===
async def main():
    await start_webserver()         # запустить слушающий порт (не блокирует)
    print("Запуск aiogram polling")
    await dp.start_polling(bot)     # polling будет работать параллельно (в том же loop)
